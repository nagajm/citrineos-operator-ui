...
import type { VsStation } from '@lib/zappo/types';
...
export const OperatorsAssign = ({ operatorId }: Props) => {
  ...
  const loadData = async () => {
    ...
    try {
      const [allRes, assignedRes] = await Promise.all([
        fetch('/api/zappo/stations'),
        fetch(`/api/zappo/operators/${operatorId}/stations`),
      ]);
  ...
  const handleAssign = async () => {
    ...
    setError('');
    try {
      const res = await fetch(`/api/zappo/operators/${operatorId}/stations`, {
        method: 'POST',
